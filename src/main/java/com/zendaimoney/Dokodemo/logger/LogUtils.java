...
public class LogUtils {
	...
	private static String generateRandomFilename(String ex) {
		String filename = Reporter.getCurrentTestResult().getName();
		// String filename = ex.replaceAll("\\s", "_").replaceAll(":",
	...
	public static void setDataPrepareFailed(String message, String exception) throws Exception {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[失败]:%s,[Error]%s", message, exception));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[失败]:%s,[Error]%s", message, exception));			
		}
	...
	public static void setDataPreparePassed(String message) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[成功]:%s", message));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[成功]:%s", message));			
		}
	...
	public static void setBrowserFailed(String type, String operation, String url, String exception) throws Exception {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[中止]%s[操作]:%s操作,[Error]%s", type, operation, exception));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[中止]%s[操作]:%s操作,[Error]%s", type, operation, exception));			
		}
	...
	public static void setBrowserWarned(String type, String operation, String url, String exception) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[警告]%s[操作]:%s操作,[Error]%s", type, operation, exception));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[警告]%s[操作]:%s操作,[Error]%s", type, operation, exception));			
		}
	...
	public static void setBrowserPassed(String type, String operation, String url) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[成功]%s[操作],%s(%s)", type, operation, url));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[成功]%s[操作],%s(%s)", type, operation, url));			
		}
	...
	public static void setBrowserPassed(String type, String operation, String model, String url) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[成功]页面模型操作,%s.(%s.class)", operation, model));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[成功]页面模型操作,%s.(%s.class)", operation, model));			
		}
	...
	public static void setBrowserFailed(String type, String operation, String model, String url, String exception) throws Exception {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[失败]页面模型操作,%s.(%s.class),[Error]%s", operation, model, exception));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[失败]页面模型操作,%s.(%s.class),[Error]%s", operation, model, exception));			
		}
	...
	public static void setElementPassed(String url, String operation, String element) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[成功]%s.%s操作", element, operation));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[成功]%s.%s操作", element, operation));			
		}
	...
	public static void setElementPassed(String url, String operation, String value, String element) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[成功]%s.%s(%s)操作", element, operation, value));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[成功]%s.%s(%s)操作", element, operation, value));			
		}
	...
	public static void setElementFailed(String url, String operation, String element, String exception) throws Exception {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[失败]%s.%s操作,[Error]%s", element, operation, exception));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[失败]%s.%s操作,[Error]%s", element, operation, exception));			
		}
	...
	public static void setElementFailed(String url, String operation, String value, String element, String exception) throws Exception {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[失败]%s.%s(%s)操作,[Error]%s", element, operation, value, exception));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[失败]%s.%s(%s)操作,[Error]%s", element, operation, value, exception));			
		}
	...
	public static void setElementEmpty(String url, String operation, String element) throws Exception {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[失败]元素找不到:%s.%s操作，找不到元素", element, operation));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[失败]元素找不到:%s.%s操作，找不到元素", element, operation));			
		}
	...
	public static void setAssertPassed(String url, String operation, String element, String expect, String actual, String message) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[成功]%s,实际值:%s, 期望值:%s", message, actual, expect));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[成功]%s,实际值:%s, 期望值:%s", message, actual, expect));			
		}
	...
	public static void setAssertFailed(String url, String operation, String element, String expect, String actual, String message) throws Exception {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[失败]%s,实际值:%s,期望值:%s", message, actual, expect));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[失败]%s,实际值:%s,期望值:%s", message, actual, expect));			
		}
	...
	public static void setSleep(String seconds) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[警告]Sleep(%s)", seconds));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[警告]Sleep(%s)", seconds));			
		}
	...
	public static void setMessage(String message) {
		if (Comm.operateStep.get(Reporter.getCurrentTestResult().getName()) == null) {
			ArrayList<String> list_data = new ArrayList<String>();
			list_data.add(String.format("[用户]Log(%s)", message));
			Comm.operateStep.put(Reporter.getCurrentTestResult().getName(), list_data);
		}else{
			Comm.operateStep.get(Reporter.getCurrentTestResult().getName()).add(String.format("[用户]Log(%s)", message));			
		}
